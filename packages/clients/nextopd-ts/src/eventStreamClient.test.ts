...
import { createNextopdEventStreamClient } from "./eventStreamClient.ts";

test("nextopd event stream client dispatches typed topic events", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const events: unknown[] = [];
  const client = createNextopdEventStreamClient({
...
test("nextopd event stream client sends and filters scoped subscriptions", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const events: unknown[] = [];
  const client = createNextopdEventStreamClient({
...
test("nextopd event stream client can opt out of a default scope", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const events: unknown[] = [];
  const client = createNextopdEventStreamClient({
    defaultScope: { workspaceId: "workspace-1" },
...
test("nextopd event stream client publishes typed intents after connect", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client fails handshake on a pre-ready error frame", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client fails handshake on unexpected pre-ready frames", async () => {
  for (const frame of [
    {
      acceptedAt: "2026-05-30T08:00:01Z",
      kind: "ack",
...
test("nextopd event stream client fails handshake on an invalid server frame", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client fails handshake on a protocol version mismatch", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client fails handshake on a catalog revision mismatch", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client tears down a failed handshake before retrying", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const client = createNextopdEventStreamClient({
    resolveUrl: () => "ws://127.0.0.1:4545/v1/events/ws?access_token=token-1",
    webSocketFactory(url) {
...
test("nextopd event stream client sends heartbeat pings after ready and clears pong timeout on pong", async () => {
  const sockets: FakeEventStreamSocket[] = [];
...
test("nextopd event stream client closes and resets the socket when pong times out", async () => {
  const sockets: FakeEventStreamSocket[] = [];
  const scheduler = new FakeHeartbeatScheduler();
  const client = createNextopdEventStreamClient({
    heartbeat: scheduler.config,
...
test("nextopd event stream client reconnects and flushes current subscriptions", async () => {
  const sockets: FakeEventStreamSocket[] = [];
