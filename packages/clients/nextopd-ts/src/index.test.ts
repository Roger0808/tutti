...
test("generated nextopd client returns parsed health response", async () => {
  const client = createClient({
    baseUrl: "http://localhost:4545/",
    fetch: async () =>
      new Response(JSON.stringify({ service: "nextopd", status: "ok" }), {
        status: 200,
  ...
  assert.deepEqual(response.data, { service: "nextopd", status: "ok" });
  assert.equal(response.error, undefined);
});

test("generated nextopd client surfaces structured protocol errors", async () => {
  const client = createClient({
    baseUrl: "http://localhost:4545",
    fetch: async () =>
      new Response(
...
test("generated nextopd client returns typed workspace lists", async () => {
  const client = createClient({
    baseUrl: "http://localhost:4545/",
    fetch: async () =>
      new Response(
...
test("shared nextopd client unwraps workspace list responses", async () => {
  const client = createNextopdClient({
    fetch: async () =>
      new Response(
        JSON.stringify({
          workspaces: [{ id: "ws-1", name: "One", lastOpenedAt: null }],
...
test("shared nextopd client forwards bearer auth tokens", async () => {
...
test("shared nextopd client creates workspace agent sessions with bearer auth", async () => {
  let authorizationHeader = "";
  let requestPath = "";
...
test("shared nextopd client lists workspace agent sessions with query params", async () => {
  let requestPath = "";
...
test("shared nextopd client deletes user projects with bearer auth", async () => {
  let authorizationHeader = "";
  let requestMethod = "";
  let requestPath = "";
  let requestBody: unknown;
...
test("shared nextopd client tracks analytics events with bearer auth", async () => {
  let authorizationHeader = "";
  let requestMethod = "";
  let requestPath = "";
  let requestBody: unknown;
...
test("shared nextopd client reads workspace file preview bytes", async () => {
  let requestMethod = "";
  let requestPath = "";
  let requestQuery = "";
...
test("shared nextopd client loads agent provider composer options", async () => {
  let requestMethod = "";
  let requestPath = "";
  let requestBody: unknown;
...
test("shared nextopd client probes agent providers", async () => {
  let requestMethod = "";
  let requestPath = "";
...
test("shared nextopd client runs agent provider actions", async () => {
  let requestMethod = "";
  let requestPath = "";
...
test("shared nextopd client deletes workspace agent sessions", async () => {
  let requestMethod = "";
  let requestPath = "";
...
test("shared nextopd client submits workspace agent interactive responses", async () => {
...
test("shared nextopd client normalizes structured protocol errors", async () => {
  const client = createNextopdClient({
    fetch: async () =>
      new Response(
        JSON.stringify({
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      assert.ok(error instanceof NextopdProtocolError);
      assert.equal(getNextopdProtocolErrorCode(error), "workspace_not_found");
...
test("normalizeNextopdError extracts structured error details", () => {
  const normalized = normalizeNextopdError({
    error: {
  ...
  assert.ok(normalized instanceof NextopdProtocolError);
...
test("normalizeNextopdError recognizes issue manager protocol codes", () => {
  const normalized = normalizeNextopdError(
    {
      error: {
...
test("getNextopdErrorI18nCandidates prefers reason-specific keys", () => {
  const candidates = getNextopdErrorI18nCandidates(
