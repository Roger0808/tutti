...
export interface CreateNextopdEventStreamClientInput {
  defaultScope?: BusinessEventScopeV1;
  resolveUrl: () => Promise<string> | string;
  webSocketFactory?: NextopdEventStreamSocketFactory;
  heartbeat?: Partial<NextopdEventStreamHeartbeatConfig>;
  reconnect?: false | Partial<NextopdEventStreamReconnectConfig>;
}
...
export interface NextopdEventStreamClient {
  ...
  subscribe<TTopic extends ServerToClientEventTopic>(
    ...
    options?: NextopdEventStreamSubscribeOptions
  ): () => void;
  subscribeConnectionState(
    listener: (state: NextopdEventStreamConnectionState) => void
  ): () => void;
}

export type NextopdEventStreamConnectionState =
  | "connected"
  | "connecting"
  | "disconnected"
  | "disposed";

export interface NextopdEventStreamSubscribeOptions {
  scope?: BusinessEventScopeV1 | null;
}

type NextopdEventStreamSocketFactory = (
  url: string
) => NextopdEventStreamSocket;

interface NextopdEventStreamSocket {
...
interface NextopdEventStreamHeartbeatConfig {
...
interface NextopdEventStreamReconnectConfig {
  initialDelayMs: number;
  maxDelayMs: number;
  scheduleTimeout: (callback: () => void, delayMs: number) => TimerCleanup;
}

const defaultHeartbeatConfig: NextopdEventStreamHeartbeatConfig = {
...
const defaultReconnectConfig: NextopdEventStreamReconnectConfig = {
  initialDelayMs: 500,
  maxDelayMs: 10_000,
  scheduleTimeout: (callback, delayMs) => {
    const handle = globalThis.setTimeout(callback, delayMs);
...
export function createNextopdEventStreamClient(
  input: CreateNextopdEventStreamClientInput
): NextopdEventStreamClient {
  const webSocketFactory =
    input.webSocketFactory ?? defaultNextopdEventStreamSocketFactory;
  const heartbeat = {
  ...
  const connectionStateListeners = new Set<
    (state: NextopdEventStreamConnectionState) => void
  >();
  const reconnect =
  ...
  let socket: NextopdEventStreamSocket | null = null;
  ...
  return {
    ...
    dispose() {
      ...
      rejectPendingPublishes(new Error("Nextopd event stream was disposed."));
    ...
    async publishIntent(topic, payload) {
      ...
      if (!activeSocket || !ready) {
        throw new Error("Nextopd event stream is not connected.");
      }
  ...
  function connectInternal(): Promise<void> {
    if (disposed) {
      return Promise.reject(new Error("Nextopd event stream was disposed."));
    }
    cancelReconnect();
    if (connectPromise) {
    ...
    connectPromise = (async () => {
      ...
      return await new Promise<void>((resolve, reject) => {
        ...
        const messageListener = (event: MessageEvent) => {
          ...
          if (!ready) {
            if (frame.kind !== "ready") {
              fail(
                frame.kind === "error"
                  ...
                  : new Error(
                      `Nextopd event stream received an unexpected ${frame.kind} frame before ready.`
                    )
              );
              return;
            }
            ...
            if (frame.protocolVersion !== businessEventProtocolVersion) {
              fail(
                new Error(
                  `Nextopd event stream protocol version mismatch. Expected ${String(businessEventProtocolVersion)}, received ${String(frame.protocolVersion)}.`
                )
              );
              return;
            }
            ...
            if (frame.catalogRevision !== businessEventCatalogRevision) {
              fail(
                new Error(
                  `Nextopd event stream catalog revision mismatch. Expected ${businessEventCatalogRevision}, received ${frame.catalogRevision}.`
                )
              );
              return;
            }
        ...
        const errorListener = () => {
          if (!ready) {
            fail(new Error("Nextopd event stream connection failed."));
          }
        };
  ...
  function resetSocketState(nextSocket: NextopdEventStreamSocket): void {
  ...
  function startHeartbeat(activeSocket: NextopdEventStreamSocket): void {
    stopHeartbeat();
  ...
  function notifyConnectionState(
    state: NextopdEventStreamConnectionState
  ): void {
...
function defaultNextopdEventStreamSocketFactory(
  url: string
): NextopdEventStreamSocket {
...
function parseServerFrame(data: unknown): ParsedServerFrame {
  if (typeof data !== "string") {
    return {
      error: new Error(
        "Nextopd event stream received a non-text server frame during handshake."
      ),
      ok: false
    };
  }
  ...
  try {
    ...
    return {
      error: new Error(
        "Nextopd event stream received an invalid server frame during handshake."
      ),
      ok: false
    };
  }
...
function getReadyCompatibilityError(frame: unknown): Error | null {
  ...
  if (frame.protocolVersion !== businessEventProtocolVersion) {
    return new Error(
      `Nextopd event stream protocol version mismatch. Expected ${businessEventProtocolVersion}, received ${String(frame.protocolVersion)}.`
    );
  }

  if (frame.catalogRevision !== businessEventCatalogRevision) {
    return new Error(
      `Nextopd event stream catalog revision mismatch. Expected ${businessEventCatalogRevision}, received ${String(frame.catalogRevision)}.`
    );
  }
...
function detachSocketListeners(
  socket: NextopdEventStreamSocket,
...
function createCloseError(event: CloseEvent): Error {
  ...
  return new Error(
    `Nextopd event stream closed (${event.code || 1006})${suffix}.`
  );
}
...
function createFrameError(
  ...
  return new Error(
    frame.message || frame.code || "Nextopd event stream error."
  );
}
