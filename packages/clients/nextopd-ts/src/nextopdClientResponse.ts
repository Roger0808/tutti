import { normalizeNextopdError } from "./errors.ts";
...
function parseNextopdError(
  error: unknown,
  statusCode: number | undefined,
  fallback: string
): Error {
  const normalizedError = normalizeNextopdError(error, statusCode ?? 0);
  if (normalizedError) {
