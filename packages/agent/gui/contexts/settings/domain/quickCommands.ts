...
function normalizeQuickCommand(value: unknown): QuickCommand | null {
  ...
  const title = normalizeTextValue(value.title).slice(
    0,
    MAX_QUICK_COMMAND_TITLE_LENGTH
  );
  ...
  if (value.kind === "terminal") {
    const command = normalizeTextValue(value.command).slice(
      0,
      MAX_QUICK_COMMAND_VALUE_LENGTH
    );
    if (command.length === 0) {
      return null;
    }
  ...
  if (value.kind === "url") {
    const url = normalizeTextValue(value.url).slice(
      0,
      MAX_QUICK_COMMAND_VALUE_LENGTH
    );
    if (url.length === 0) {
