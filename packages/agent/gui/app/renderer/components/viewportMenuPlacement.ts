...
export function placeViewportSubmenuAtItem(options: {
  ...
  const preferredRight =
    options.parentMenuRect.left + options.parentMenuRect.width + gap;
  const preferredLeft =
    options.parentMenuRect.left - gap - options.submenuSize.width;
  const fitsRight =
    preferredRight + options.submenuSize.width <=
