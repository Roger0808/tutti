import type { JSX, SVGProps } from "react";

/**
 * Lined plus (24x24 viewBox). Fill uses `currentColor` for ghost icon buttons.
 * Source asset: `assets/icons/add-lined.svg`.
 */
export function AddLinedIcon(props: SVGProps<SVGSVGElement>): JSX.Element {
  "use memo";
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
      {...props}
    >
      <path d="M12 1.5C12.8284 1.5 13.5 2.17157 13.5 3V10.5H21C21.8284 10.5 22.5 11.1716 22.5 12C22.5 12.8284 21.8284 13.5 21 13.5H13.5V21C13.5 21.8284 12.8284 22.5 12 22.5C11.1716 22.5 10.5 21.8284 10.5 21V13.5H3C2.17157 13.5 1.5 12.8284 1.5 12C1.5 11.1716 2.17157 10.5 3 10.5H10.5V3C10.5 2.17157 11.1716 1.5 12 1.5Z" />
    </svg>
  );
}
