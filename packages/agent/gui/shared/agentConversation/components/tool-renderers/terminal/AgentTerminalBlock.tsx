...
export function AgentTerminalBlock({
  ...
  const disclosureButton =
    outputLines.length > MAX_OUTPUT_LINES ? (
      <button
        type="button"
        className="mt-2 flex w-fit items-center border-0 bg-transparent p-0 text-left text-[11px] font-medium text-[var(--accent)] transition-colors hover:underline focus-visible:underline focus-visible:outline-none"
  ...
  return (
    <div className="workspace-agents-status-panel__detail-tool-terminal overflow-hidden rounded-[8px] border border-[var(--line-2)] bg-[var(--background-panel)]">
      {command ? (
        <div
          className={`flex min-w-0 items-center gap-3 px-3 text-[12px] text-[var(--text-secondary)] ${
            hasOutput
      ...
      {hasOutput ? (
        <AgentToolScrollArea
          ...
          viewportClassName={`px-3 py-2 text-[12px] leading-5 ${
