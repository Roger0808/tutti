...
export type IpcInspectorKind = "invoke" | "send";
export type IpcInspectorStatus = "pending" | "ok" | "error" | "sent";

export interface IpcInspectorRecord {
  recordId: string;
  kind: IpcInspectorKind;
  channel: string;
  payload: unknown;
  status: IpcInspectorStatus;
  startedAt: number;
  completedAt: number;
  durationMs: number;
  result?: unknown;
...
export interface IpcInspectorExportResult {
  content: string;
}
