...
func TestNextopdBlackBoxHealthAndEmptyCatalog(t *testing.T) {
	daemon := startTestDaemon(t)

	health := mustRequestJSON[nextopgenerated.HealthStatusResponse](t, daemon, http.MethodGet, "/v1/health", nil, http.StatusOK)
	if health.Service != "nextopd" {
		t.Fatalf("health.service = %q, want %q", health.Service, "nextopd")
	}
	if health.Status != nextopgenerated.Ok {
		t.Fatalf("health.status = %q, want %q", health.Status, nextopgenerated.Ok)
	}

	workspaces := mustRequestJSON[nextopgenerated.ListWorkspacesResponse](t, daemon, http.MethodGet, "/v1/workspaces", nil, http.StatusOK)
	...
	startup := mustRequestJSON[nextopgenerated.StartupWorkspaceResponse](t, daemon, http.MethodGet, "/v1/workspaces/startup", nil, http.StatusOK)
	...
	workspacesAfterStartup := mustRequestJSON[nextopgenerated.ListWorkspacesResponse](t, daemon, http.MethodGet, "/v1/workspaces", nil, http.StatusOK)
	...
	dbPath := filepath.Join(daemon.stateDir, "nextopd.db")
...
func TestNextopdBlackBoxWorkspaceLifecycle(t *testing.T) {
	daemon := startTestDaemon(t)

	created := mustRequestJSON[nextopgenerated.WorkspaceResponse](t, daemon, http.MethodPost, "/v1/workspaces", nextopgenerated.CreateWorkspaceRequest{
		Name: "Workspace One",
	}, http.StatusCreated)
	...
	listed := mustRequestJSON[nextopgenerated.ListWorkspacesResponse](t, daemon, http.MethodGet, "/v1/workspaces", nil, http.StatusOK)
	...
	updated := mustRequestJSON[nextopgenerated.WorkspaceResponse](t, daemon, http.MethodPatch, "/v1/workspaces/"+created.Workspace.Id, nextopgenerated.UpdateWorkspaceRequest{
	...
	fetched := mustRequestJSON[nextopgenerated.WorkspaceResponse](t, daemon, http.MethodGet, "/v1/workspaces/"+created.Workspace.Id, nil, http.StatusOK)
	...
	startupBeforeOpen := mustRequestJSON[nextopgenerated.StartupWorkspaceResponse](t, daemon, http.MethodGet, "/v1/workspaces/startup", nil, http.StatusOK)
	...
	opened := mustRequestJSON[nextopgenerated.WorkspaceResponse](t, daemon, http.MethodPost, "/v1/workspaces/"+created.Workspace.Id+"/open", nil, http.StatusOK)
	...
	startupAfterOpen := mustRequestJSON[nextopgenerated.StartupWorkspaceResponse](t, daemon, http.MethodGet, "/v1/workspaces/startup", nil, http.StatusOK)
...
func startTestDaemon(t *testing.T) *testDaemon {
	...
	logPath := filepath.Join(stateDir, "logs", "nextopd.log")
...
func mustBuildDaemonBinary(t *testing.T) string {
	...
	buildBinaryOnce.Do(func() {
		tempDir, err := os.MkdirTemp("", "nextopd-blackbox-bin-")
		if err != nil {
		...
		if err != nil {
			buildBinaryErr = fmt.Errorf("build nextopd binary: %w\n%s", err, strings.TrimSpace(string(output)))
		}
	})

	if buildBinaryErr != nil {
		t.Fatalf("build nextopd binary: %v", buildBinaryErr)
	}
...
func waitForHealth(t *testing.T, daemon *testDaemon) {
	...
	for time.Now().Before(deadline) {
		if daemon.cmd.ProcessState != nil && daemon.cmd.ProcessState.Exited() {
			t.Fatalf("nextopd exited before becoming healthy: %v\nstdout:\n%s\nstderr:\n%s", lastErr, daemon.stdout.String(), daemon.stderr.String())
		}

		health, err := requestJSON[nextopgenerated.HealthStatusResponse](daemon, http.MethodGet, "/v1/health", nil)
		if err == nil && health.Status == nextopgenerated.Ok {
			return
		}
		lastErr = err
	...
	t.Fatalf("timed out waiting for nextopd health: %v\nstdout:\n%s\nstderr:\n%s", lastErr, daemon.stdout.String(), daemon.stderr.String())
}
...
func waitForListenerInfo(t *testing.T, daemon *testDaemon) string {
	...
	deadline := time.Now().Add(daemonStartTimeout)
	listenerInfoPath := filepath.Join(daemon.stateDir, "run", "nextopd.listener.json")
	var lastErr error

	for time.Now().Before(deadline) {
		if daemon.cmd.ProcessState != nil && daemon.cmd.ProcessState.Exited() {
			t.Fatalf("nextopd exited before publishing listener info: %v\nstdout:\n%s\nstderr:\n%s", lastErr, daemon.stdout.String(), daemon.stderr.String())
		}
	...
	t.Fatalf("timed out waiting for nextopd listener info: %v\nstdout:\n%s\nstderr:\n%s", lastErr, daemon.stdout.String(), daemon.stderr.String())
	return ""
}
...
func requestJSONWithStatus[T any](daemon *testDaemon, method string, path string, body any) (T, int, error) {
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		var failure nextopgenerated.ApiErrorResponse
...
func stopTestDaemon(t *testing.T, daemon *testDaemon) {
	...
	if err := daemon.cmd.Process.Signal(syscall.SIGINT); err != nil && !errors.Is(err, os.ErrProcessDone) {
		t.Fatalf("signal nextopd shutdown: %v", err)
	}
