...
import (
	...
	eventprotocol "github.com/tutti-os/tutti/services/nextopd/api/events/generated"
...
func TestNextopdBlackBoxEventStreamWebSocketRejectsMissingAccessToken(t *testing.T) {
...
func TestNextopdBlackBoxEventStreamWebSocketReadyAndValidation(t *testing.T) {
	daemon := startTestDaemon(t)
...
func TestNextopdBlackBoxEventStreamPreferenceIntentPublishesUpdatedEvent(t *testing.T) {
	daemon := startTestDaemon(t)

	initialPreferences := mustRequestJSON[nextopgenerated.DesktopPreferencesStateResponse](
		t,
		daemon,
		http.MethodGet,
	...
	after := mustRequestJSON[nextopgenerated.DesktopPreferencesStateResponse](
		t,
		daemon,
		http.MethodGet,
		"/v1/preferences/desktop",
	...
	if after.Preferences.Locale != nextopgenerated.ZhCN {
		t.Fatalf("stored locale = %q, want %q", after.Preferences.Locale, nextopgenerated.ZhCN)
	}
	if after.Preferences.DefaultAgentProvider != nextopgenerated.Codex {
		t.Fatalf("stored defaultAgentProvider = %q, want %q", after.Preferences.DefaultAgentProvider, nextopgenerated.Codex)
	}
	if after.Preferences.DockPlacement != nextopgenerated.Bottom {
		t.Fatalf("stored dockPlacement = %q, want %q", after.Preferences.DockPlacement, nextopgenerated.Bottom)
	}
	if after.Preferences.ThemeSource != nextopgenerated.DesktopThemeSourceDark {
		t.Fatalf("stored themeSource = %q, want %q", after.Preferences.ThemeSource, nextopgenerated.DesktopThemeSourceDark)
	}
}
