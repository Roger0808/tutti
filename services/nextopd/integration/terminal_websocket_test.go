...
func TestNextopdBlackBoxWorkspaceTerminalWebSocketStreamsInputAndOutput(t *testing.T) {
	...
	createdWorkspace := mustRequestJSON[nextopgenerated.WorkspaceResponse](
		t,
		daemon,
		http.MethodPost,
		"/v1/workspaces",
		nextopgenerated.CreateWorkspaceRequest{
			Name: "Workspace Terminal",
		},
		http.StatusCreated,
	)

	createdTerminal := mustRequestJSON[nextopgenerated.WorkspaceTerminalResponse](
		t,
		daemon,
		http.MethodPost,
		"/v1/workspaces/"+createdWorkspace.Workspace.Id+"/terminals",
		nextopgenerated.CreateWorkspaceTerminalRequest{},
		http.StatusCreated,
	)
	...
	for {
		...
		resized := mustRequestJSON[nextopgenerated.WorkspaceTerminalResponse](
			t,
			daemon,
			http.MethodGet,
...
func TestNextopdBlackBoxWorkspaceTerminalWebSocketExitFrameCarriesExitCode(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Windows terminal support needs ConPTY-specific implementation")
	}
	...
	readyMarker := "nextop-terminal-exit-ready"
	readyPayload, err := json.Marshal(map[string]string{
		"type": "input",
		"data": "printf 'nextop-terminal-%s\\n' 'exit-ready'\r",
	})
	if err != nil {
