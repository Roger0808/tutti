...
func TestRotatingFileWriterRotatesActiveLogWithDateIndex(t *testing.T) {
	...
	writer, err := NewRotatingFileWriter(filepath.Join(dir, "nextopd.log"), RotatingFileWriterOptions{
		MaxSizeBytes: 5,
		MaxBackups:   10,
		MaxAgeDays:   14,
	...
	rotatedPath := filepath.Join(dir, "nextopd.2026-05-05.log")
	activePath := filepath.Join(dir, "nextopd.log")
...
func TestRotatingFileWriterRotatesOnCalendarDayChange(t *testing.T) {
	...
	rotated, err := os.ReadFile(filepath.Join(dir, "nextopd.2026-05-05.log"))
	if err != nil {
		t.Fatalf("read rotated file: %v", err)
	}
...
func TestRotatingFileWriterPrunesDirectoryBudget(t *testing.T) {
	dir := t.TempDir()
	oldPath := filepath.Join(dir, "nextopd.2026-05-04.log")
	newPath := filepath.Join(dir, "nextopd.2026-05-05.1.log")
