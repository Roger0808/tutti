...
import (
	...
	builtinapps "github.com/tutti-os/tutti/services/nextopd/builtin-apps"
...
func TestAppFactoryServiceCreateUsesDraftDirAndReferenceContext(t *testing.T) {
	...
	if strings.Contains(prompt, "<nextop_app_factory_context>") {
		t.Fatalf("initial prompt should not inline factory context:\n%s", prompt)
	}
	for _, forbidden := range []string{
		"Create this Nextop workspace app from the user request below.",
		"User request:",
		"Factory context:",
		"App Factory Context",
	} {
	...
	if mentionContext.Task != "Create a Nextop workspace app package under the output packageRoot directory." {
...
func TestAppFactoryValidationRejectsAgentsFileWithOnlyRuntimeManagedBlock(t *testing.T) {
  ...
  "runtime": {
	...
	managedOnly := nextopRuntimeManagedBlockBegin + "\n# Nextop Runtime\n" + nextopRuntimeManagedBlockEnd + "\n"
	...
	if strings.Contains(string(content), "NEXTOP-RUNTIME") {
...
func TestAppFactoryServiceBumpsManifestVersionAfterRepublish(t *testing.T) {
	...
	if err := appStore.PutAppPackage(ctx, workspacebiz.AppPackage{
		...
		ManifestJSON: `{"schemaVersion":"nextop.app.manifest.v1","appId":"app_1","version":"0.1.0","name":"Test App","description":"Test app","icon":{"type":"","src":""},"runtime":{"kind":"custom","bootstrap":"bootstrap.sh","healthcheckPath":"/healthz"}}`,
	...
	manifest, _, err := workspacebiz.ReadAppManifestFile(filepath.Join(draftDir, appFactoryPackageRootRelativePath, "nextop.app.json"))
	if err != nil {
...
func TestAppFactoryServicePublishRecoversExistingPackageForSameJob(t *testing.T) {
	...
	if err := appStore.PutAppPackage(ctx, workspacebiz.AppPackage{
		...
		PackageDir:           packageDir,
		Manifest:             testAppManifest("app_1", "0.1.0"),
		ManifestJSON:         `{"schemaVersion":"nextop.app.manifest.v1","appId":"app_1","version":"0.1.0","name":"Test App","description":"Test app","runtime":{"bootstrap":"bootstrap.sh","healthcheckPath":"/healthz"}}`,
		Source:               workspacebiz.AppPackageSourceGenerated,
		FactoryJobID:         "job-1",
