...
func resolveExtractedPackageRoot(stagingDir string) (string, error) {
	if _, err := os.Stat(filepath.Join(stagingDir, "nextop.app.json")); err == nil {
	...
	for _, entry := range entries {
		...
		if _, err := os.Stat(filepath.Join(candidate, "nextop.app.json")); err == nil {
	...
	if len(packageRoots) != 1 {
		return "", errors.New("app archive must contain exactly one package root with nextop.app.json")
	}
