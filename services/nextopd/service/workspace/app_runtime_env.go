...
const nextopAppRuntimeRootEnv = "NEXTOP_APP_RUNTIME_ROOT"
const nextopAppRuntimeCacheRootEnv = "NEXTOP_APP_RUNTIME_CACHE_ROOT"
const nextopAppRuntimeCatalogEnv = "NEXTOP_APP_RUNTIME_CATALOG"
const appRuntimeCatalogSchemaVersion = "nextop.app.runtimes.v2"
const appRuntimeBaselineProfile = "baseline"
const defaultNextopAppRuntimeCatalogURL = "https://d1x7gb6wqsqmnm.cloudfront.net/nextop-app-runtimes/catalog.json"
...
func (r DefaultManagedAppRuntimeResolver) Resolve(ctx context.Context) (ResolvedAppRuntime, error) {
	...
	if root == "" {
		root = strings.TrimSpace(envValue(r.environ(), nextopAppRuntimeRootEnv))
	}
	if root == "" {
...
func (r DefaultManagedAppRuntimeResolver) resolvedRuntime(root string) (ResolvedAppRuntime, error) {
	...
	envOverrides := []string{
		nextopAppRuntimeRootEnv + "=" + root,
		"NEXTOP_APP_PYTHON=" + python,
		"NEXTOP_APP_NODE=" + node,
		"NEXTOP_APP_NPM=" + npm,
...
func (r DefaultManagedAppRuntimeResolver) ensureRuntime(ctx context.Context, root string) error {
	...
	if catalogSource == "" {
		return fmt.Errorf("managed app runtime is unavailable at %s and %s is not configured", root, nextopAppRuntimeCatalogEnv)
	}
...
func (r DefaultManagedAppRuntimeResolver) runtimeCatalogSource() string {
	for _, item := range r.environ() {
		...
		if ok && key == nextopAppRuntimeCatalogEnv {
			return strings.TrimSpace(value)
		}
	}
	return defaultNextopAppRuntimeCatalogURL
}
...
func defaultManagedAppRuntimeRoot(env []string) string {
	cacheRoot := strings.TrimSpace(envValue(env, nextopAppRuntimeCacheRootEnv))
	if cacheRoot == "" {
		cacheRoot = filepath.Join(nextoptypes.DefaultStateDir(), "app-runtimes")
	}
