...
const (
	...
	interruptedFactoryJobReason     = "App Factory job was interrupted because Nextop stopped before it finished."
	nextopRuntimeManagedBlockBegin  = "<!-- BEGIN NEXTOP-RUNTIME (auto-managed; do not edit) -->"
	nextopRuntimeManagedBlockEnd    = "<!-- END NEXTOP-RUNTIME -->"
)
...
func prepareAppFactoryJob(ctx context.Context, job workspacebiz.AppFactoryJob) error {
	...
	envOverrides := []string{
		"NEXTOP_APP_ID=" + job.AppID,
		"NEXTOP_APP_PACKAGE_DIR=" + draftPackageDir,
		"NEXTOP_APP_RUNTIME_DIR=" + job.RuntimeDir,
		"NEXTOP_APP_DATA_DIR=" + job.DataDir,
		"NEXTOP_APP_LOG_DIR=" + job.LogDir,
	}
...
func (s *AppFactoryService) validatePackage(ctx context.Context, workspaceID string, job workspacebiz.AppFactoryJob) error {
	...
	manifestPath := filepath.Join(draftPackageDir, "nextop.app.json")
...
func readCleanAppFactoryAgentsFile(job workspacebiz.AppFactoryJob) (string, error) {
	...
	cleaned := stripNextopRuntimeManagedBlock(string(data))
...
func stripNextopRuntimeManagedBlock(value string) string {
	for {
		begin := strings.Index(value, nextopRuntimeManagedBlockBegin)
		if begin < 0 {
			return strings.TrimSpace(value)
		}
		endSearchStart := begin + len(nextopRuntimeManagedBlockBegin)
		end := strings.Index(value[endSearchStart:], nextopRuntimeManagedBlockEnd)
		if end < 0 {
			return strings.TrimSpace(value)
		}
		end += endSearchStart + len(nextopRuntimeManagedBlockEnd)
...
func (s *AppFactoryService) stateDir() string {
	...
	if value := strings.TrimSpace(os.Getenv("NEXTOP_STATE_DIR")); value != "" {
		return value
	}
	return nextoptypes.DefaultStateDir()
}
