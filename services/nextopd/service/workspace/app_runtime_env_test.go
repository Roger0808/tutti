...
func TestDefaultManagedAppRuntimeResolverInjectsBaselineRuntime(t *testing.T) {
	...
	env := workspaceAppProcessEnv(append([]string{"NEXTOP_APP_ID=test"}, resolved.EnvOverrides...)...)
	...
	if appRuntimeEnvValue(env, "NEXTOP_APP_PYTHON") != filepath.Join(pythonBinDir, appRuntimePythonBinaryName()) {
		t.Fatalf("NEXTOP_APP_PYTHON = %q", appRuntimeEnvValue(env, "NEXTOP_APP_PYTHON"))
	}
	if appRuntimeEnvValue(env, "NEXTOP_APP_NODE") != filepath.Join(nodeBinDir, appRuntimeNodeBinaryName()) {
		t.Fatalf("NEXTOP_APP_NODE = %q", appRuntimeEnvValue(env, "NEXTOP_APP_NODE"))
	}
...
func TestDefaultManagedAppRuntimeResolverRejectsMissingRuntime(t *testing.T) {
	_, err := DefaultManagedAppRuntimeResolver{
		Environ: func() []string {
			return []string{
				nextopAppRuntimeCacheRootEnv + "=" + t.TempDir(),
				nextopAppRuntimeCatalogEnv + "=",
			}
		},
	}.Resolve(context.Background())
	if err == nil {
...
func TestDefaultManagedAppRuntimeResolverUsesDaemonCacheRoot(t *testing.T) {
	...
	root := defaultManagedAppRuntimeRoot([]string{
		nextopAppRuntimeCacheRootEnv + "=" + cacheRoot,
	})
...
func TestDefaultManagedAppRuntimeResolverUsesDefaultCatalogWhenUnset(t *testing.T) {
	...
	if source != defaultNextopAppRuntimeCatalogURL {
		t.Fatalf("runtimeCatalogSource() = %q, want %q", source, defaultNextopAppRuntimeCatalogURL)
	}
}
  ...
  "runtimes": {
	...
	resolved, err := DefaultManagedAppRuntimeResolver{
		Environ: func() []string {
			return []string{
				nextopAppRuntimeCacheRootEnv + "=" + cacheRoot,
				nextopAppRuntimeCatalogEnv + "=" + catalogPath,
				"PATH=/usr/bin:/bin",
			}
		},
	}.Resolve(context.Background())
