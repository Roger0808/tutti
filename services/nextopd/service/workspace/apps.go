...
import (
	...
	appcliservice "github.com/tutti-os/tutti/services/nextopd/service/cli/appcli"
)
...
func (s *AppCenterService) InitBuiltinPackages(ctx context.Context) error {
	...
	for _, builtin := range builtins {
		...
		manifest, manifestJSON, err := workspacebiz.ReadAppManifestFile(filepath.Join(packageDir, "nextop.app.json"))
		if err != nil {
...
func (s *AppCenterService) startPackage(ctx context.Context, workspaceID string, appPackage workspacebiz.AppPackage, restart bool) (workspacebiz.AppRuntimeState, error) {
	...
	if err != nil {
		slog.Warn("workspace app root resolution failed; app will start without NEXTOP_WORKSPACE_ROOT", "workspaceId", workspaceID, "appId", appPackage.AppID, "error", err)
	}
