...
func TestTerminalServiceCreatesLocalPTYAndSnapshotsOutput(t *testing.T) {
	...
	initialInput := "printf nextop-terminal-test\\n\r"
	...
	for time.Now().Before(deadline) {
		...
		if strings.Contains(snapshot.Data, "nextop-terminal-test") {
			break
		}
...
func TestTerminalServiceAttachStreamExitEventCarriesExitCode(t *testing.T) {
	...
	if err := service.Write(context.Background(), "ws-1", session.ID, "printf 'nextop-terminal-%s\\n' 'exit-ready'\r"); err != nil {
