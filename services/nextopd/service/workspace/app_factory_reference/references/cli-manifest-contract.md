# Nextop App CLI Manifest Contract

Create `nextop.cli.json` only when `nextop.app.json` declares:
...
- `description` is optional. When present, it describes the CLI scope as a whole for app-level discovery surfaces such as `nextop --help` and `@app` mentions.
...
- Handler `kind` must be `http`, `method` must be `POST`, and `path` must start with `/nextop/cli/`.
- Do not declare host, port, or full URLs; Nextop routes to the app runtime port.
