# Nextop App Manifest Contract

Create `nextop.app.json` in the package root with this shape:
...
- Do not include `runtime.kind`; Nextop manages the runtime baseline outside the app package.
- `runtime.bootstrap` must be a relative package path.
- `runtime.healthcheckPath` must start with `/`.
- `cli` is optional. Include it only when the app exposes commands through the Nextop CLI.
- `cli.manifest` must be a relative package path to a `nextop.app.cli.v1` manifest, usually `nextop.cli.json`.
