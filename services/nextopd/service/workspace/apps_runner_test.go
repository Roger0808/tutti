...
`), 0o755); err != nil {
	...
	t.Setenv(nextopAppRuntimeRootEnv, createManagedAppRuntimeFixture(t, root))
	t.Setenv("NEXTOP_ENV", "production")
	t.Setenv("NEXTOP_STATE_DIR", stateRoot)
	...
	wantCLIPath := filepath.Join(stateRoot, "bin", "nextop")
	if probeValues["nextopCli"] != wantCLIPath {
		t.Fatalf("probe[nextopCli] = %q, want %q", probeValues["nextopCli"], wantCLIPath)
	}
	pathDirs := filepath.SplitList(probeValues["path"])
	if len(pathDirs) == 0 || pathDirs[0] != filepath.Dir(wantCLIPath) {
		t.Fatalf("probe[path] = %q, want nextop CLI shim dir first", probeValues["path"])
	}
	...
	if !strings.Contains(string(logData), "nextop workspace app startup") || !strings.Contains(string(logData), "workspaceRoot="+root) {
...
`), 0o755); err != nil {
	...
	if got := strings.Count(string(logData), "nextop workspace app startup"); got != 2 {
...
func TestAppRunnerStartsAppWithManagedNodeRuntimeEnv(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(packageDir, "bootstrap.sh"), []byte(`#!/bin/sh
set -eu
exec "$NEXTOP_APP_NODE" "$NEXTOP_APP_PACKAGE_DIR/server.js"
`), 0o755); err != nil {
		t.Fatalf("WriteFile(bootstrap.sh) error = %v", err)
	}
	...
	t.Setenv(nextopAppRuntimeRootEnv, runtimeRoot)
	runner := &AppRunner{HealthcheckTimeout: 10 * time.Second}
	state, err := runner.Start(context.Background(), AppStartInput{
		WorkspaceID:     "ws-fnm",
...
func TestNextopCLIShimPathUsesProductionCommand(t *testing.T) {
	stateDir := t.TempDir()
	t.Setenv("NEXTOP_STATE_DIR", stateDir)
	t.Setenv("NEXTOP_ENV", "production")

	want := filepath.Join(stateDir, "bin", "nextop")
	if got := nextopCLIShimPathForPlatform("darwin"); got != want {
		t.Fatalf("nextopCLIShimPathForPlatform() = %q, want %q", got, want)
	}
}

func TestNextopCLIShimPathUsesDevelopmentCommand(t *testing.T) {
	stateDir := t.TempDir()
	t.Setenv("NEXTOP_STATE_DIR", stateDir)
	t.Setenv("NEXTOP_ENV", "development")

	want := filepath.Join(stateDir, "bin", "nextop-dev")
	if got := nextopCLIShimPathForPlatform("darwin"); got != want {
		t.Fatalf("nextopCLIShimPathForPlatform() = %q, want %q", got, want)
	}
}

func TestNextopCLIShimPathUsesWindowsCommandExtension(t *testing.T) {
	stateDir := t.TempDir()
	t.Setenv("NEXTOP_STATE_DIR", stateDir)
	t.Setenv("NEXTOP_ENV", "production")

	want := filepath.Join(stateDir, "bin", "nextop.cmd")
	if got := nextopCLIShimPathForPlatform("windows"); got != want {
		t.Fatalf("nextopCLIShimPathForPlatform() = %q, want %q", got, want)
	}
}
...
exec "`+pythonPath+`" "$NEXTOP_APP_PACKAGE_DIR/server.py"
`), 0o755); err != nil {
...
func pythonAppReadyServerScript(healthcheckPath string, writeProbe bool) string {
	...
	if writeProbe {
		...
		probeWrite = `        with open(os.path.join(os.environ["NEXTOP_APP_DATA_DIR"], "probe.json"), "w") as f:
            json.dump({
                "cwd": os.getcwd(),
...
server.bind(("127.0.0.1", int(os.environ["NEXTOP_APP_PORT"])))
server.listen(16)
