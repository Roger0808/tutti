...
func TestAppCenterServiceInitializesBuiltinCatalogAndInstallState(t *testing.T) {
	t.Setenv("NEXTOP_APP_CATALOG_URL", "")
	...
	if automation.Manifest.Author == nil || automation.Manifest.Author.Name != "Nextop" || len(automation.Manifest.Tags) != 2 || !strings.Contains(automation.ManifestJSON, `"tags"`) {
	...
	if _, err := os.Stat(filepath.Join(automation.PackageDir, "nextop.cli.json")); err != nil {
...
func TestAppCenterServiceInitializesBuiltinPackagesWhenRemoteCatalogFails(t *testing.T) {
	t.Setenv("NEXTOP_APP_CATALOG_FILE", "")
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		http.Error(writer, "unavailable", http.StatusServiceUnavailable)
	}))
	t.Cleanup(server.Close)
	t.Setenv("NEXTOP_APP_CATALOG_URL", server.URL+"/catalog.json")
...
func TestAppCenterServiceListsRemoteBuiltinBeforeDownloadAndMaterializesOnDemand(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(appPackage.PackageDir, "nextop.app.json")); err != nil {
...
`)
	if err := os.WriteFile(filepath.Join(packageDir, "nextop.app.json"), data, 0o644); err != nil {
...
func mustReadManifestForTest(t *testing.T, packageDir string) workspacebiz.AppManifest {
	t.Helper()
	manifest, _, err := workspacebiz.ReadAppManifestFile(filepath.Join(packageDir, "nextop.app.json"))
	if err != nil {
