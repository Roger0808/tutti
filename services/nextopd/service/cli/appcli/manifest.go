...
const (
	ManifestSchemaVersion = "nextop.app.cli.v1"
...
type ManifestCommandOutput struct {
	DefaultMode cliservice.OutputMode `json:"defaultMode"`
	JSON        bool                  `json:"json"`
...
func validateHandler(handler ManifestCommandHandler, location string) error {
	...
	if !strings.HasPrefix(handlerPath, "/nextop/cli/") {
		return fmt.Errorf("cli manifest %s.path must start with /nextop/cli/", location)
	}
