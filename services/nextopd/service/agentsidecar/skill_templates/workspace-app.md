---
name: workspace-app
description: Use for `mention://workspace-app?...` links to discover, inspect, or invoke CLI-enabled Nextop workspace app commands.
---
...
Use the injected `nextop-cli` skill as the command reference for CLI syntax and available commands. This skill owns workspace app mention interpretation and decides how to use that CLI reference.
...
After reading the mention query, recover the smallest useful app context through Nextop CLI:

1. Read the injected `nextop-cli` command guide and find commands whose description says they are provided by the mentioned workspace app.
2. If several apps have similar names, match by `appId` from the mention, not only by the visible label.
3. Use the listed `nextop <scope> <command>` examples to inspect or invoke the app.
