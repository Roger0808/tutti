...
func defaultRuntimeEnv(input PrepareInput, stateDir string) []string {
	env := []string{
		"NEXTOP_WORKSPACE_ID=" + strings.TrimSpace(input.WorkspaceID),
		"NEXTOP_AGENT_SESSION_ID=" + strings.TrimSpace(input.AgentSessionID),
		"NEXTOP_AGENT_PROVIDER=" + strings.TrimSpace(input.Provider),
	}
