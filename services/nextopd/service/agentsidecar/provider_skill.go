...
func nextopCLISkill(input PrepareInput) string {
	return renderProviderSkillTemplate(
		"skill_templates/nextop-cli.md",
		map[string]string{
...
func providerSkills(input PrepareInput) []providerSkillSpec {
	skills := []providerSkillSpec{
		{
			baseName: nextopSkillName,
			files:    map[string]string{"SKILL.md": nextopCLISkill(input)},
		},
		{
...
func installProviderSkillFiles(skillPath string, spec providerSkillSpec) error {
	if err := os.MkdirAll(skillPath, 0o755); err != nil {
		return fmt.Errorf("create nextop provider skill directory: %w", err)
	}
	...
	for relativePath, content := range spec.files {
		...
		if err := os.MkdirAll(filepath.Dir(targetPath), 0o755); err != nil {
			return fmt.Errorf("create nextop provider skill file directory: %w", err)
		}
		if err := os.WriteFile(targetPath, []byte(content), 0o644); err != nil {
			return fmt.Errorf("write nextop provider skill: %w", err)
		}
	}
	return nil
}
...
func allocateSkillName(root string, baseName string) (string, error) {
	...
	candidates := []string{baseName, baseName + "-nextop"}
	for index := 2; index <= 99; index++ {
		candidates = append(candidates, fmt.Sprintf("%s-nextop-%d", baseName, index))
	}
