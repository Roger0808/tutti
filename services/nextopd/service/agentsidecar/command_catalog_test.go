...
func TestCommandGuideFromCapabilitiesUsesRelevantRegistryCommands(t *testing.T) {
	guide := commandGuideFromCapabilities("nextop", []cliservice.Capability{
		{
			ID:          "diagnostics.ping",
	...
	if !strings.Contains(guide, "nextop issue get --issue-id <issue-id>") {
		t.Fatalf("guide missing issue get: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue topic list") {
		t.Fatalf("guide missing topic list: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue list --topic-id <topic-id>") {
		t.Fatalf("guide missing topic-scoped issue list: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue update --issue-id <issue-id> --status completed --json") {
		t.Fatalf("guide missing issue update: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue task update --issue-id <issue-id> --task-id <task-id> --status completed --json") {
		t.Fatalf("guide missing issue task update: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue task create --issue-id <issue-id> --title <title>") ||
		!strings.Contains(guide, "without creating a run") {
		t.Fatalf("guide missing breakdown task create guidance: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue task run create --agent-provider <agent-provider> --agent-session-id <agent-session-id> --issue-id <issue-id> --task-id <task-id> --json") {
		t.Fatalf("guide missing issue task run create: %q", guide)
	}
	if !strings.Contains(guide, "nextop issue task run complete --issue-id <issue-id> --run-id <run-id> --status <status> --task-id <task-id> --summary <summary>") {
		t.Fatalf("guide missing issue task run complete: %q", guide)
	}
	if !strings.Contains(guide, "nextop agent sessions") {
...
func TestCommandGuideFromCatalogPassesAgentRuntimeContext(t *testing.T) {
	...
	guide := commandGuideFromCatalog(context.Background(), &catalog, "workspace-1", "nextop")
	if catalog.context.Source != "agent-runtime" || catalog.context.WorkspaceID != "workspace-1" {
		t.Fatalf("context = %#v", catalog.context)
	}
	if !strings.Contains(guide, "nextop issue list") {
...
func TestCommandGuideFromCatalogUsesProvidedCLIName(t *testing.T) {
	...
	guide := commandGuideFromCatalog(context.Background(), &catalog, "workspace-1", "nextop-dev")

	if !strings.Contains(guide, "nextop-dev issue list") {
		t.Fatalf("guide = %q, want nextop-dev command", guide)
	}
	if strings.Contains(guide, "`nextop issue list`") {
...
func TestFallbackCommandGuideUsesProvidedCLIName(t *testing.T) {
	guide := commandGuide(PrepareInput{CLICommand: "nextop-dev"})

	if !strings.Contains(guide, "nextop-dev issue topic list") {
		t.Fatalf("guide = %q, want nextop-dev topic list fallback command", guide)
	}
	if !strings.Contains(guide, "nextop-dev issue list --topic-id <topic-id>") {
		t.Fatalf("guide = %q, want nextop-dev fallback command", guide)
	}
	if !strings.Contains(guide, "nextop-dev issue update --issue-id <issue-id> --status completed --json") {
		t.Fatalf("guide = %q, want nextop-dev issue update fallback command", guide)
	}
	if !strings.Contains(guide, "nextop-dev issue task run create --issue-id <issue-id> --task-id <task-id> --agent-provider <provider> --agent-session-id <session-id> --json") {
		t.Fatalf("guide = %q, want nextop-dev issue task run create fallback command", guide)
	}
	if !strings.Contains(guide, "nextop-dev issue task create --issue-id <issue-id> --title <title> --content <content> --json") ||
		!strings.Contains(guide, "persist child tasks without creating a run") {
		t.Fatalf("guide = %q, want nextop-dev breakdown task create fallback command", guide)
	}
	if !strings.Contains(guide, "nextop-dev issue task run complete --issue-id <issue-id> --task-id <task-id> --run-id <run-id> --status completed") {
		t.Fatalf("guide = %q, want nextop-dev issue task run complete fallback command", guide)
	}
}
...
func TestFallbackCommandGuideDefaultsToProductionCLIName(t *testing.T) {
	...
	if !strings.Contains(guide, "nextop issue topic list") {
		t.Fatalf("guide = %q, want nextop topic list command", guide)
	}
	if !strings.Contains(guide, "nextop issue list --topic-id <topic-id>") {
		t.Fatalf("guide = %q, want nextop command", guide)
	}
	if strings.Contains(guide, "nextop-dev") {
