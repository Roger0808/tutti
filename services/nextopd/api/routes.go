...
func RegisterRoutes(mux *http.ServeMux, routes Routes) {
	wrapper := &nextopgenerated.ServerInterfaceWrapper{
		Handler:          routes,
	...
	mux.HandleFunc("/v1/workspaces/{workspaceID}", func(w http.ResponseWriter, r *http.Request) {
		workspaceID := nextopgenerated.WorkspaceID(r.PathValue("workspaceID"))
		switch r.Method {
		case http.MethodGet:
	...
	mux.HandleFunc("/v1/workspaces/{workspaceID}/open", func(w http.ResponseWriter, r *http.Request) {
		...
		routes.OpenWorkspace(w, r, nextopgenerated.WorkspaceID(r.PathValue("workspaceID")))
	})
}
