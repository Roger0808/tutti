...
func TestDaemonAPIRoutesCheckUserProjectPath(t *testing.T) {
	...
	RegisterRoutes(mux, NewRoutes(DaemonAPI{
		UserProjectService: stubUserProjectService{
			checkPathFn: func(_ context.Context, input userprojectservice.CheckPathInput) (userprojectservice.PathCheck, error) {
				if input.Path != "/workspace/nextop" {
					t.Fatalf("path = %q, want /workspace/nextop", input.Path)
				}
	...
	var response nextopgenerated.UserProjectPathCheckResponse
	decodeGeneratedRouteResponse(t, recorder, &response)
	if !response.Exists || !response.IsDirectory || response.Path != "/workspace/nextop" {
...
func TestDaemonAPIRoutesUserProjects(t *testing.T) {
	...
	var response nextopgenerated.UserProjectListResponse
	decodeGeneratedRouteResponse(t, recorder, &response)
	if len(response.Projects) != 1 {
		t.Fatalf("projects length = %d, want 1", len(response.Projects))
	}
	if response.Projects[0].Path != "/workspace/nextop" {
		t.Fatalf("path = %q, want /workspace/nextop", response.Projects[0].Path)
	}
}
...
func TestDaemonAPIRoutesUseUserProject(t *testing.T) {
	...
	var response nextopgenerated.UserProjectResponse
	decodeGeneratedRouteResponse(t, recorder, &response)
	if response.Project.Path != "/workspace/nextop" {
		t.Fatalf("path = %q, want /workspace/nextop", response.Project.Path)
	}
}
