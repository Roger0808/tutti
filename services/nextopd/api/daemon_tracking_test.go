...
func TestTrackEventsAcceptsEvents(t *testing.T) {
	...
	request := nextopgenerated.TrackEventsRequestObject{
		Body: &nextopgenerated.TrackEventsRequest{
			Events: []nextopgenerated.TrackEvent{
				{
	...
	if _, ok := response.(nextopgenerated.TrackEvents202Response); !ok {
		t.Fatalf("response = %T, want %T", response, nextopgenerated.TrackEvents202Response{})
	}
...
func TestTrackEventsRejectsInvalidEventShape(t *testing.T) {
	validEvent := nextopgenerated.TrackEvent{
		Name:     "workspace.opened",
		ClientTs: 1749124800000,
	}
	tooManyEvents := make([]nextopgenerated.TrackEvent, 101)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, ok := response.(nextopgenerated.TrackEvents400JSONResponse); !ok {
				t.Fatalf("response = %T, want %T", response, nextopgenerated.TrackEvents400JSONResponse{})
			}
...
func trackEventsResponseContains(t *testing.T, response nextopgenerated.TrackEventsResponseObject, text string) bool {
	t.Helper()
