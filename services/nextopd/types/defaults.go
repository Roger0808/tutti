...
func ResolveAnalyticsConfig() AnalyticsConfig {
	return AnalyticsConfig{
		...
		AppKey:        resolveStringOverride("NEXTOP_ANALYTICS_APP_KEY", generatedDefaults.Analytics.AppKey),
		Channel:       generatedDefaults.Analytics.Channel,
		ChannelDomain: resolveStringOverride("NEXTOP_ANALYTICS_CHANNEL_DOMAIN", generatedDefaults.Analytics.ChannelDomain),
...
func resolveLogsDir(stateRootDir string) string {
	override := strings.TrimSpace(os.Getenv("NEXTOP_LOG_DIR"))
	if override != "" {
		return override
	}
...
func resolveDBPath(stateRootDir string) string {
	override := strings.TrimSpace(os.Getenv("NEXTOPD_DB_PATH"))
	if override != "" {
		return override
	}
...
func resolveDaemonLogPath(logsDir string) string {
	override := strings.TrimSpace(os.Getenv("NEXTOPD_LOG_PATH"))
	if override != "" {
		return override
	}
...
func resolveDesktopLogPath(logsDir string) string {
	override := strings.TrimSpace(os.Getenv("NEXTOP_DESKTOP_LOG_PATH"))
	if override != "" {
		return override
	}
...
func resolvePIDPath(runDir string) string {
	override := strings.TrimSpace(os.Getenv("NEXTOPD_PID_PATH"))
	if override != "" {
		return override
	}
...
func resolveTCPAddr() string {
	override := strings.TrimSpace(os.Getenv("NEXTOPD_ADDR"))
	if override != "" {
		return override
	}
...
func resolveAnalyticsDisabled() bool {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("NEXTOP_ANALYTICS_DISABLED")))
	switch value {
	case "":
		return false
	case "1", "true", "yes":
...
func resolveAnalyticsDebug() bool {
	return resolveNextopEnv() == "development"
}

func resolveAnalyticsAppID() int {
	override := strings.TrimSpace(os.Getenv("NEXTOP_ANALYTICS_APP_ID"))
	if override == "" {
...
func resolveAnalyticsAppVersion() string {
	override := strings.TrimSpace(os.Getenv("NEXTOP_ANALYTICS_APP_VERSION"))
	if override != "" {
		return override
	}
	return resolveStringOverride("NEXTOP_APP_VERSION", generatedDefaults.Analytics.AppVersion)
}
