...
This file applies to `services/nextopd/*`.

`nextopd` is the primary business core of the repository.
...
- `nextopd` owns agent provider availability, agent session workflows, and AgentGUI launch intent publication.
- `apps/cli` must remain a thin daemon client for `nextop agent`; it should parse terminal shape and render daemon output, not call desktopd, renderer-private APIs, or agent runtimes directly.
- AgentGUI launch requests from CLI or other local integrations should enter `nextopd` first and be published as business events on `/v1/events/ws`.
...
- daemon-owned local state defaults to `~/.nextop` in production and `~/.nextop-dev` in development
- prefer deriving new daemon-owned file paths from `NEXTOP_STATE_DIR` helpers instead of writing directly under `$HOME`
