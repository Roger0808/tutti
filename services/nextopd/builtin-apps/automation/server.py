...
PACKAGE_DIR = Path(os.environ["NEXTOP_APP_PACKAGE_DIR"])
DATA_DIR = Path(os.environ["NEXTOP_APP_DATA_DIR"])
LOG_DIR = Path(os.environ["NEXTOP_APP_LOG_DIR"])
RUNTIME_DIR = Path(os.environ["NEXTOP_APP_RUNTIME_DIR"])
WORKSPACE_ROOT = os.environ.get("NEXTOP_WORKSPACE_ROOT", "").strip()
WORKSPACE_ID = os.environ["NEXTOP_WORKSPACE_ID"]
WORKSPACE_NAME = os.environ.get("NEXTOP_WORKSPACE_NAME", WORKSPACE_ID)
...
def nextop_cli_command():
    configured = os.environ.get("NEXTOP_CLI", "").strip()
    if not configured:
        raise RuntimeError("NEXTOP_CLI is not configured")
    return configured


def run_nextop_cli(args, timeout=60, log_file=None):
    command_path = nextop_cli_command()
    ...
    try:
        ...
        raise RuntimeError(f"NEXTOP_CLI executable was not found: {command_path}") from exc
    if log_file:
        if result.stdout:
    ...
    if result.returncode != 0:
        message = (result.stderr or result.stdout or "nextop cli command failed").strip()
...
def cancel_agent_session(agent_session_id):
    if not agent_session_id:
        return
    run_nextop_cli(["agent", "cancel", "--session-id", agent_session_id], timeout=30)


def open_agent_session(agent_session_id):
    if not agent_session_id:
        raise ValueError("run does not have an agent session")
    return run_nextop_cli(["agent", "open", "--session-id", agent_session_id], timeout=30)


def agent_session_messages(agent_session_id, log_file=None):
    result = run_nextop_cli(
...
def agent_providers_payload():
    result = run_nextop_cli(["agent", "providers"], timeout=30)
    providers = []
...
def agent_composer_options_payload(provider, locale=None):
    ...
    result = run_nextop_cli(args, timeout=30)
    effective_settings = (
...
def automation_completion_command(run):
    cli = shlex.quote(nextop_cli_command())
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_write(self, method):
        try:
            path = urlparse(self.path).path
            if method == "POST" and path.startswith("/nextop/cli/"):
            ...
            if urlparse(self.path).path.startswith("/nextop/cli/"):
    ...
    def handle_cli(self, path):
        ...
        if path == "/nextop/cli/complete-run":
...
def main():
    ...
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    host = os.environ.get("NEXTOP_APP_HOST", "127.0.0.1")
    port = int(os.environ["NEXTOP_APP_PORT"])
