...
def load_server_module(temp_root):
    ...
    os.environ["NEXTOP_APP_PACKAGE_DIR"] = str(package_dir)
    os.environ["NEXTOP_APP_DATA_DIR"] = str(temp_root / "data")
    os.environ["NEXTOP_APP_LOG_DIR"] = str(temp_root / "logs")
    os.environ["NEXTOP_APP_RUNTIME_DIR"] = str(temp_root / "runtime")
    os.environ["NEXTOP_WORKSPACE_ID"] = "workspace-1"
    os.environ["NEXTOP_APP_PORT"] = "0"
    os.environ["NEXTOP_CLI"] = "/usr/local/bin/nextop"
...
class RunnerOptionsPayloadTest(unittest.TestCase):
    def test_runner_options_uses_cli_locale_and_structured_configs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def fake_run_nextop_cli(args, timeout=30, log_file=None):
                calls.append(args)
            ...
            with mock.patch.object(module, "run_nextop_cli", fake_run_nextop_cli):
