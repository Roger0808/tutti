...
import (
	...
	"github.com/tutti-os/tutti/services/nextopd/service/cli/appcli"
)
...
func TestCatalogKeepsEmbeddedAppsWhenRemoteURLFails(t *testing.T) {
	...
	} else if app.Manifest.CLI == nil || app.Manifest.CLI.Manifest != "nextop.cli.json" {
		t.Fatalf("embedded automation cli manifest = %#v, want nextop.cli.json", app.Manifest.CLI)
	}
