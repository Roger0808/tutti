...
//go:embed automation/COMMANDS.md automation/bootstrap.sh automation/icon.png automation/nextop.app.json automation/nextop.cli.json automation/server.py automation/locales/zh-CN/manifest.json automation/static
var files embed.FS
...
const (
	remoteCatalogSchemaVersionV1 = "nextop.app.catalog.v1"
	remoteCatalogFileEnv         = "NEXTOP_APP_CATALOG_FILE"
	remoteCatalogURLEnv          = "NEXTOP_APP_CATALOG_URL"
	defaultRemoteCatalogURL      = "https://d1x7gb6wqsqmnm.cloudfront.net/nextop-app-releases/catalog.json"
...
func readBuiltinManifest(sourceDir string) (workspacebiz.AppManifest, error) {
	data, err := files.ReadFile(filepath.Join(sourceDir, "nextop.app.json"))
	if err != nil {
