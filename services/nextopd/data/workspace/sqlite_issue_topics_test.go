...
func TestSQLiteStoreIssueTopicsMigrationRepairsMissingMarker(t *testing.T) {
	...
	if _, err := store.db.ExecContext(ctx, `
DELETE FROM nextopd_schema_migrations
WHERE id = ?;
INSERT INTO nextopd_schema_migrations (id, applied_at_unix_ms)
...
func createLegacyIssueV2Database(t *testing.T, dbPath string) {
	...
	_, err = db.Exec(`
CREATE TABLE nextopd_schema_migrations (
  id TEXT PRIMARY KEY,
  applied_at_unix_ms INTEGER NOT NULL
);
INSERT INTO nextopd_schema_migrations (id, applied_at_unix_ms) VALUES
  ('workspaces_v1', 1),
  ('workspaces_v2', 1),
  ('workspaces_v3', 1),
  ('workspaces_v4', 1),
