...
test("workspace settings agent panel lists agent controls", () => {
  assert.match(
    source,
    /function WorkspaceAgentSettingsSection[\s\S]*workspace\.externalImport\.settingsLabel[\s\S]*workspace\.settings\.general\.agentWorkModeLabel[\s\S]*workspace\.settings\.general\.defaultAgentProviderLabel[\s\S]*workspace\.settings\.general\.browserUseConnectionModeLabel[\s\S]*<ComputerUseSetupRow/
  );
  assert.match(source, /role="radiogroup"/);
  assert.match(source, /role="radio"/);
  assert.match(source, /aria-checked=\{selected\}/);
  assert.match(source, /desktopAgentWorkModes\.map/);
  assert.match(source, /onAgentWorkModeChange\(mode\)/);
});
