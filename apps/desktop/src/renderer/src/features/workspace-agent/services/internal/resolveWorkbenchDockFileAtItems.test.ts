...
import { resolveWorkbenchDockFileAtItems } from "./resolveWorkbenchDockFileAtItems.ts";
...
test("resolveWorkbenchDockFileAtItems returns open file preview nodes in stack order", () => {
  const items = resolveWorkbenchDockFileAtItems({
    host: createHost({
...
test("resolveWorkbenchDockFileAtItems includes image previews restored from snapshot state", () => {
  const items = resolveWorkbenchDockFileAtItems({
    host: createHost({
...
test("resolveWorkbenchDockFileAtItems reads preview targets from runtime node state", () => {
  const items = resolveWorkbenchDockFileAtItems({
    host: createHost({
