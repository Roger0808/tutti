...
先说健康的部分：运行时适配层不需要重构。`standard_acp_adapter.go` 是共享引擎；走标准 ACP 的 provider 通过 descriptor 的 runtime strategy 提供 command、mode 映射与环境配置，OpenCode 迁移后已不再保留 provider 专属的生产 ACP 配置文件。需要修的是适配层之外的两处。
...
provider 切片（依赖 P1 的能力描述符 schema）：🚧 通用描述符类型与消费适配层已经落地。✅ Codex 与 OpenCode 已完成单 provider 收口：两者的 runtime/status/installer/auth watcher、composer/model/capability catalog、turn lifecycle 投影、系统 target、事件归一化均由 descriptor 驱动；OpenCode 的标准 ACP command、permission mode 映射和 spawn-time model env 由通用 ACP descriptor strategy 构造，GUI 的 model image gating 只读 `modelImageInputRequired` capability，不再按 OpenCode 名称猜测；identity/target/icon key 生成物及漂移检查覆盖两个已迁移 provider，并同时校验全语言 locale key 与完整图标资源，未知图标不再静默回退。其余 provider 的迁移、注册表与全量枚举最终相等校验，以及未迁移 provider 的 legacy fallback 清理尚未完成。
