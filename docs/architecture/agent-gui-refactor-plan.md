...
provider 切片（依赖 P1 的能力描述符 schema）：🚧 通用描述符类型与消费适配层已经落地。✅ Codex 已完成单 provider 收口：runtime/status/installer/auth watcher、model/skill/capability/slash catalog、turn lifecycle 投影、系统 target、事件归一化均由 descriptor 驱动；GUI 的 plan/权限/skill/slash 行为改读能力与目录契约；identity/target/icon key 生成物及漂移检查已接入，未知图标不再静默回退。其余 provider 的迁移、注册表与全量枚举最终相等校验，以及未迁移 provider 的 legacy fallback 清理尚未完成。
