...
The `stable` tag must be recreated as an annotated tag pointing at the concrete stable release commit. GitHub's Releases page uses the release `created_at` value for ordering, and lightweight tags that point at the stable commit inherit that older commit timestamp instead of the refresh time.
