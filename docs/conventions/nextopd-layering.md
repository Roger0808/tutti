...
This document defines the internal layering rules for `services/nextopd`.

## Purpose

`nextopd` is the primary business core for the open-source local product.

Its directory structure should follow a clear layered desktop-daemon model,
while remaining scaled to the current size of `nextopd`.
...
- starting a real `nextopd` process
...
When reviewing `nextopd` changes, ask:
