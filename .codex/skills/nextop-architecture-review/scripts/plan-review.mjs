...
const USAGE = `Usage: node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs [options]
...
function resolveOutputPath(options) {
  ...
  return resolve(
    tmpdir(),
    `nextop-architecture-review-${timestamp}.${extension}`
  );
}
...
function classifyArea(path) {
  ...
  if (path.startsWith("services/nextopd/")) return "services/nextopd";
...
function buildContext(changedFiles, preflightSignals) {
  const primaryAreas = new Set(
    changedFiles
      ...
      .map((file) => {
        ...
        if (file.path.startsWith("services/nextopd/"))
          return "services/nextopd";
  ...
  const hasNextopd = primaryAreas.has("services/nextopd");
  ...
  const hasGeneratedClient = changedFiles.some((file) =>
    file.path.startsWith("packages/clients/nextopd-ts/src/generated/")
  );
  ...
  const crossCuttingReasons = [
    hasDesktop && hasNextopd ? "desktop and nextopd changed together" : null,
    hasOpenApi && (hasGeneratedClient || hasDesktopBridge)
      ? "daemon contract and consumers changed together"
      : null,
    hasPackages && (hasDesktop || hasNextopd)
...
function buildPrompt(
  ...
  return `You are the ${taskDefinition.title} for a nextop architecture review.
...
function renderMarkdown(packageJson) {
  const lines = [
    "# Nextop Architecture Review Tasks",
    "",
...
function renderSummary(packageJson) {
  const lines = [
    "# Nextop Architecture Review Summary",
    "",
    packageJson.workflowEntry?.packagePath
      ? `Task package: ${packageJson.workflowEntry.packagePath}`
      : null,
...
function collectPreflightSignals(changedFiles, options) {
  ...
  const hasGeneratedDaemon = changedFiles.some((file) =>
    file.path.startsWith("services/nextopd/api/generated/")
  );
  const hasGeneratedClient = changedFiles.some((file) =>
    file.path.startsWith("packages/clients/nextopd-ts/src/generated/")
  );
  ...
  if ((hasGeneratedDaemon || hasGeneratedClient) && !hasOpenApi) {
    signals.push({
      ...
      evidence:
        "services/nextopd/api/generated or packages/clients/nextopd-ts/src/generated changed while services/nextopd/api/openapi/nextopd.v1.yaml did not."
    });
  }
...
function isCrossAreaImport(path, line) {
  ...
  if (path.startsWith("apps/desktop/")) {
    return /["'](?:services\/|packages\/(?!clients\/nextopd-ts|ui\/system))/.test(
      line
    );
  }
  if (path.startsWith("services/nextopd/")) {
