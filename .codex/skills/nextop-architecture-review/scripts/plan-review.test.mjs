...
test("plans tasks, risk, and preflight signals from a real git diff", async () => {
  ...
  await writeWorkspaceFile(
    ...
    '{ "name": "@nextop/workbench-surface", "version": "0.0.1" }\n'
  );
...
test("--from-package with --format summary does not require a git repository", async () => {
  const workspaceRoot = await mkdtemp(join(tmpdir(), "nextop-review-package-"));
    ...
    JSON.stringify(
      {
        ...
        tasks: [sampleTask("desktop-layering"), sampleTask("nextopd-layering")],
        empty: false
      },
      null,
      2
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Nextop Architecture Review Summary/);
  assert.match(result.stdout, /\| desktop-layering \|/);
  assert.doesNotMatch(result.stdout, /\| nextopd-layering \|/);
...
test("--scope-file narrows diff review to matching scoped paths", async () => {
  ...
  await writeWorkspaceFile(
    workspaceRoot,
    "packages/workbench/surface/package.json",
    '{ "name": "@nextop/workbench-surface", "version": "0.0.2" }\n'
  );
  await writeFile(
    scopePath,
    JSON.stringify(
...
test("--scope-file falls back to scoped files when diff overlap is empty", async () => {
  ...
  runGit(workspaceRoot, [
    "-c",
    "user.name=Nextop Test",
    "-c",
    "user.email=nextop-test@example.com",
    "commit",
    "-m",
    "update renderer app"
  ]);
...
async function createFixtureRepo() {
  const workspaceRoot = await mkdtemp(join(tmpdir(), "nextop-review-"));
