...
test("collapses sibling file candidates into directory scopes", async () => {
  const workspaceRoot = await mkdtemp(join(tmpdir(), "nextop-review-scope-"));
  ...
  await writeWorkspaceFile(
    workspaceRoot,
    "services/nextopd/service/workspaces/manager.go",
    "package workspaces\n"
  );
    ...
    JSON.stringify(
      {
        ...
        candidates: [
          ...
          {
            path: "services/nextopd/service/workspaces/manager.go",
            reason: "path segment matched workspaces"
          }
        ]
      },
