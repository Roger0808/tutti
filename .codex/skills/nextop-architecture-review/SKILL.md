---
name: nextop-architecture-review
description: Review nextop git diffs for project structure, layering, module ownership, and duplicate event-center infrastructure by planning focused architecture review tasks, then having the main agent orchestrate sub-agents for only the changed areas.
---

# Nextop Architecture Review

Use this skill when reviewing a `nextop` change or a named module for repository structure, module ownership, or layering compliance. This is a focused architecture review, not a general bug hunt.
...
2. When the user names a module, let the main agent infer a few candidate path keywords and gather candidate paths. Then normalize those paths into a scope file:
   ...
   ```bash
   node ./.codex/skills/nextop-architecture-review/scripts/build-review-scope.mjs \
     --input /tmp/nextop-review-candidates.json \
     --output /tmp/nextop-review-scope.json
   ```
...
3. Run the review planner from the repository root:
   ...
   ```bash
   node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs \
     --scope-file /tmp/nextop-review-scope.json \
     --format json \
     --output-temp
   ```
...
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --format markdown
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --format summary
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --base origin/main
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --staged
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --no-untracked
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --scope-file /tmp/nextop-review-scope.json --format summary
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --scope-file /tmp/nextop-review-scope.json --scope-mode static-only --format summary
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --task desktop-layering --format markdown
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --output /tmp/nextop-review-tasks.json
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --output-temp
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --from-package /tmp/nextop-review-tasks.json --format markdown
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --from-package /tmp/nextop-review-tasks.json --task desktop-layering --format summary
```
...
```bash
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --format json --output-temp
```
...
```bash
node ./.codex/skills/nextop-architecture-review/scripts/plan-review.mjs --from-package /tmp/nextop-architecture-review-YYYYMMDDTHHMMSSZ.json --format markdown
```
...
```bash
node --test ./.codex/skills/nextop-architecture-review/scripts/plan-review.test.mjs ./.codex/skills/nextop-architecture-review/scripts/build-review-scope.test.mjs
```

Read `references/nextop-layering.md` when a task needs the compact project rules.
