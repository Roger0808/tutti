...
- `services/nextopd` owns business rules, durable local state, domain workflows, and persistence.
...
- Does the change put business logic in `services/nextopd` rather than `apps/desktop`?
...
- `docs/conventions/nextopd-layering.md`
- `services/nextopd/AGENTS.md`
...
- Before adding local event buses, event centers, product pub-sub routes, or
  ...
  the seam.
- Shared default-source changes under `config/nextop.defaults.json` require generated outputs to be refreshed.
